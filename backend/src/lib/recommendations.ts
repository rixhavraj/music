...
export function applyQueueRules(
  ...
  options: {
    ...
    prioritizeLiked?: boolean;
    maxConsecutiveSameArtist?: number;
  } = {}
): Track[] {
  const {
    respectSkips = true,
    prioritizeLiked = true,
    maxConsecutiveSameArtist = 2,
  } = options;

  const skipped = respectSkips ? getFrequentlySkipped(userId) : new Set<string>();
  const liked = prioritizeLiked ? new Set(getLikedTrackIds(userId)) : new Set<string>();
...
export function generateRecommendations(
  ...
  const recentIds = new Set(getRecentlyPlayed(userId, 30));
  const skippedIds = getFrequentlySkipped(userId);
  const likedIds = new Set(getLikedTrackIds(userId));

  // Helpers
  const fresh = (t: Track) => !recentIds.has(t.id) && !skippedIds.has(t.id);
  const liked = (t: Track) => likedIds.has(t.id);
  const recent = (t: Track) => recentIds.has(t.id);
  ...
  const partyKeywords = [...MOOD_KEYWORDS.party, ...MOOD_KEYWORDS.energetic];
