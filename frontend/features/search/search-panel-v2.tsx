...
export function SearchPanelV2({ onPlay }: SearchPanelV2Props) {
  ...
  return (
    <div className="flex flex-col gap-4 pb-28">
      {/* Search bar */}
      <div className={`flex items-center gap-3 rounded-full bg-white/10 border px-4 py-3 transition ${isListening ? "border-[#1db954] shadow-[0_0_16px_#1db95440]" : "border-white/10 focus-within:border-white/40"}`}>
        <Search size={20} className="shrink-0 text-[#b3b3b3]" />
        <input
          ref={inputRef}
          value={query}
        ...
        <button
          ...
          className={`p-1.5 rounded-full transition ${isListening ? "bg-[#1db954] text-black animate-pulse" : "text-gray-400 hover:text-white hover:bg-white/10"}`}
          title="Voice search"
        >
          <Mic size={16} />
        </button>
      ...
      {showEmpty ? (
        <div className="flex flex-col gap-6">
          ...
          {/* Browse genres */}
          <div>
            <h3 className="font-bold text-base mb-3">Browse by Genre</h3>
            <div className="grid grid-cols-3 gap-3">
              {[
                ...
                <button
                  ...
                  className={`h-20 rounded-xl bg-gradient-to-br ${genre.color} flex items-end p-3 hover:brightness-110 transition cursor-pointer`}
                >
                  <span className="font-bold text-sm text-white drop-shadow">{genre.name}</span>
                </button>
              ))}
            </div>
          </div>
